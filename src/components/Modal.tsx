...
const Modal = ({ isOpen, onClose }: any) => {
  ...
  return (
    <div className="fixed top-0 left-0 w-screen h-screen z-50 flex items-center justify-center">
      <div
        className={twMerge(
          "w-[1217px] h-[675px] relative rounded shadow bg-stone-100 flex items-center justify-center gap-32 lg:w-11/12 lg:gap-10 sm:h-screen sm:w-full sm:flex-col sm:gap-0"
        )}
      >
        <button onClick={closeModal}>
          <IconClose className="absolute top-8 right-8 z-50" />
        </button>
